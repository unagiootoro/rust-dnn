...
fn run<B: Backend>(device: Device<B>) -> Result<()> {
...

    {
        for epoch in 0..max_epochs {
            ...
            let vae_decoder_safetensors = serialize(model.dec.all_parameters_map())?;
            let path = format!("../exclude/vae_decoder_epoch{}.safetensors", epoch);
            fs::write(path, vae_decoder_safetensors).unwrap();
        }
    }

    {
        // let z_dim = 32;
        let mut decoder = Decoder::new(z_dim, device)?;

        let epoch = 0;
        let path = format!("../exclude/vae_decoder_epoch{}.safetensors", epoch);
        let vae_decoder_safetensors = fs::read(path).unwrap();
        decoder.load_parameters_map(deserialize::<B, f32>(vae_decoder_safetensors, device)?)?;
