...
impl<B: Backend> Block<B> {
    pub fn new(
        ...
        let ln1 = RMSNorm::new(n_embd, 1e-5, device);
        // let attn = MultiHeadAttention::new(n_embd, n_head, dropout_ratio, use_bias, device);
        let attn = Attention::new(n_embd, n_head, n_head / 4, dropout_ratio, use_bias, device);
        // let ln2 = LayerNorm::new(vec![n_embd], 1e-5, use_bias, device);
        let ln2 = RMSNorm::new(n_embd, 1e-5, device);
